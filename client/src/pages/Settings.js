...
function Settings() {
  ...
  // Modal state
  const [modal, setModal] = useState(null); // { type: 'email'|'password'|'username'|'name' }
  ...
  useEffect(() => {
    async function fetchUser() {
      ...
      if (!token) {
        setStatus({ type: "error", message: "You must be logged in to view Settings." });
        setLoading(false);
        return;
      }
      try {
  ...
  async function saveModal() {
    ...
    if (modal.type === "email") {
      ...
      if (!modalForm.currentPassword) {
        setStatus({ type: "error", message: "Current password is required to change email." });
        return;
      }
